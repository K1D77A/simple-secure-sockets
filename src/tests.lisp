...
(defun send-packets-to-recipients (list-genned-by-n-lots)
  (let ((data (make-string 10 :initial-element #\o)))
    (lparallel:pmapcar (lambda (cons)
                         (let ((con (car cons))
                               (clients (cdr cons)))
                           (mapcar (lambda (recipient)
                                     (send con (build-data-packet recipient data)))
                                   clients)))
                       list-genned-by-n-lots)
    t))
(defun gen-and-send-packets (clients n)
  (let* ((n-calc (ceiling (sqrt n)))
         (lst (n-lots-of-x-random-recipients clients n-calc n-calc)))
